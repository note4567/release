...
def get_urls():
    ...
    try:
        # input 要素を取得
        input_element = selm.get_element('//input[@class="shop-input-input js-keyword-input"]')
        # input 要素に " "(スペース)を入力
        # 但しこのサイトの場合は '' の中に " " を記入する必要がある。
        input_element.send_keys('" "')
        # button 要素を取得(検索ボタン)
        button_element = selm.get_element('//button[@class="shop-input-btn js-keyword-btn"]')
        ...
        for _ in range(1000):
            ...
            # 詳細ページのリンクを取得
            link_elements = selm.get_elements('//a[@class="shop-favList-title-name mod-btn_left rt_cf_s_href"]')
            for link_element in link_elements:
                ...
                yield url
            # nextページを取得
            if next_element := selm.get_element('//a[@class="mod-pager-list-next rt_bn_shops_list_page-next"]'):
                link_page = next_element.get_attribute("href")
...
def fetch_shop(url):
    try:
        ...
        if "/petshop/" in url:
            print("[petshop]")
            data = {"住所": bs.get_element('//div[@class="shop_info"]//ul//li[contains(text(),"〒")][1]'),
                    "TEL": bs.get_element('//div[@class="shop_info"]//ul//li[contains(text(),"TEL")][1]'),
                    "FAX": bs.get_element('//div[@class="shop_info"]//ul//li[contains(text(),"FAX")][1]'),
                    "営業時間": bs.get_element('//div[@class="shop_info"]//ul//li[contains(text(),"営業時間")][1]'),
                    "交通アクセス": "",
                    "決済手段": "",
                    "店内施設・施設情報": "",
                    "事業所名": bs.get_element('//ul//h3')}
            data_list.append(data)
            return data_list
