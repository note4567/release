#!/usr/bin/env python
""" main() の中で writerows() により複数行を書き込む"""
...
def fetch_page(url):
    ...
    try:
        ...
        for element in elements:
            ...
            data_list.append([val for val in data.values()])
            counter += 1
        return data_list
    finally:
        selm.close()
...
def main():
    with open('./bengoshi.csv', 'w') as f:
        writer = csv.writer(f)
        keys = ["弁護士氏名", "登録番号", "所属弁護士会", "法律事務所名", "懲戒種別", "自由と正義掲載年度", "処分理由の要旨"]
        writer.writerow(keys)
        for data_list in map(fetch_page, get_urls()):
            print(len(data_list))
